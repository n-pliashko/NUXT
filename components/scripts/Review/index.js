...
export default {
  ...
  methods: {
    loadItem: function (itemId) {
      let self = this
      this.$http.get(this.apiHost + config.prefix + config.reviews.reviewItem, Object.assign({}, this.requestOptions, {params: {item_id: itemId}}))
        .then(data => {
    ...
    commitMethod: function () {
      let self = this
      Vue.http.post(this.apiHost + config.prefix + config.reviews.reviewCreate, self.formr).then(function (response) {
