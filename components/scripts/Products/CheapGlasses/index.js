...
export default {
  ...
  methods: {
    ...
    loadItems (append = false) {
      ...
      return this.$http.post(this.apiHost + config.prefix + config.products.cheapGlasses, data, Object.assign({}, requestOpt, this.requestOptions)).then(response => response.json())
        .then(json => {
          if (json.items) {
    ...
    handleScroll (event) {
      ...
      if (el && !this.scrollLoad && window.scrollY >= el.offsetTop - el.offsetHeight) {
    ...
    chooseLenses (id) {
      ...
      let data = {
        ...
        'option': {
          ...
          lang: option.lang,
          image_src: option.src,
          image_type: option.type,
          image_order: option.image_order,
          eld: frameSize.eld,
          gtin: frameSize.gtin,
          arm: frameSize.arm,
    ...
    showFullsizeImage (id) {
      this.modalData = Object.values(this.items.data[id].options)[0]
    },
